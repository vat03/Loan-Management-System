    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers(HttpMethod.GET, "/api/loan-schemes/all").hasAnyRole("CUSTOMER", "ADMIN")
                .requestMatchers("/api/loan-schemes/**").hasRole("ADMIN")
                .requestMatchers(HttpMethod.GET, "/api/loan-payments/totalPaymentAmount/*/amount").hasRole("CUSTOMER")
                .requestMatchers(HttpMethod.POST, "/api/loan-payments/*/repay").hasRole("CUSTOMER")
                .requestMatchers(HttpMethod.POST, "/api/loan-payments/complete").hasRole("CUSTOMER")
                .anyRequest().authenticated())
            .addFilterBefore(jwtAuthenticationFilter, UsernamePasswordAuthenticationFilter.class);
